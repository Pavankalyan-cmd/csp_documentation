...
const models = [
  ...
  const handleProcessDocument = async () => {
    ...
    try {
      ...
      const response = await fetch(`${url}/process-document?document_url=${encodeURIComponent(documentUrl)}&template_id=${encodeURIComponent(selectedTemplateId)}&model_id=${encodeURIComponent(selectedModelId)}`, {
        method: 'POST',
        headers: {
            ...
            {progress > 0 && progress < 100 && (
              ...
              >
                <div className="text-sm text-gray-700 mb-1">
                  Sending document for processing...{progress.toFixed(1)}%
                </div>
                ...
                <div className="text-xs text-gray-500">
                  Processing time: {Math.floor(elapsedTime / 60)}m{" "}
                  {String(elapsedTime % 60).padStart(2, "0")}s
                </div>
              </motion.div>
            )}
          </AnimatePresence>
